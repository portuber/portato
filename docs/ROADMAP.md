...
**Phases 0–46 are all `[x]`; Phase 47 (Windows SCM autostart) is `[~]` in progress, and Phase 48 (importing existing forwards from `~/.ssh/config`) is planned next.** The roadmap's core is complete, including ProxyJump (jump hosts), `~/.ssh/config` alias resolution, shell completion, and tunnel tags/groups; the rest of the backlog is in [Post-1.0 candidate features](#post-10-candidate-features). The stability
